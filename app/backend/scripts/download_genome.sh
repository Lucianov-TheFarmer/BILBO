...
tmux new-session -d -s "$TMUX_SESSION_NAME" "
    ...
    if [ -f \"\$gff_FILE\" ]; then
        mv \"\$gff_FILE\" \"$GENOME_DIR/genomic.gff\" &&
        echo 'Renamed \$gff_FILE to genomic.gff.' >> $LOG_FILE
    else
        echo 'Error: .gff file not found for $ACCESSION.' >> $LOG_FILE
    fi &&
