...
tmux new-session -d -s QC_$user_id "fastqc -o $output_dir ../users/$user_id/samples/$sra_code_base/$sra_code > $log_file 2>&1; exit_code=\$?; if [ \$exit_code -eq 0 ]; then echo 'Quality analysis completed successfully' >> $log_file; curl -X POST http://bioinfo-container:8000/quality_analysis/update_status -H 'Content-Type: application/x-www-form-urlencoded' -d 'sra_code=$sra_code&status=Completed'; else echo 'Quality analysis failed with exit code \$exit_code' >> $log_file; fi; tmux wait-for -S quality_analysis_done"
